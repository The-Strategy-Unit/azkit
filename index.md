...
[azkit](https://the-strategy-unit.github.io/azkit/) enables access to an
...
Return a list of all available containers in your default Azure storage
with:
...
reading functions to bring data into R from `.parquet`, `.rds`, `.json`
or `.csv` files. For example:
...
To read in any file from the container in raw format, to be passed to
the handler of your choice, use:
...
⚠️ These values are sensitive and should not be exposed to anyone
outside The Strategy Unit. Make sure you include `.Renviron` in [the
`.gitignore`
file](https://docs.github.com/en/get-started/getting-started-with-git/ignoring-files)
for your project.

Your `.Renviron` file can contain the variables below. Ask a member of
[the Data Science
team](https://the-strategy-unit.github.io/data_science/about.html) for
...
> \[!NOTE\] Users at The Strategy Unit: when initially authenticating
> with Azure, you will be sent to the web. Since you may be logged into
> one of multiple Microsoft tenants, it is important that you
> authenticate with the same account (MLCSU) that you use for access to
> Azure storage. If you authenticate when logged into your nhs.net
> Microsoft account in your browser, your token will not be useable for
> access to Azure.
...
[`AzureRMR::get_azure_login()`](https://rdrr.io/pkg/AzureRMR/man/azure_login.html)
or
[`AzureRMR::list_azure_tokens()`](https://rdrr.io/pkg/AzureAuth/man/get_azure_token.html)
may also be helpful for troubleshooting - try them and see if they work
/ what they reveal.
...
passing in the full and correct filepath relative to the root directory
of the container.

If
[`read_azure_json()`](https://the-strategy-unit.github.io/azkit/reference/read_azure_json.md)
and similar are not working as expected, try reading in the raw data
first with
[`azkit::read_azure_file()`](https://the-strategy-unit.github.io/azkit/reference/read_azure_file.md)
and then passing that to a handler function of your choice.

## Getting help

Please use the
[Issues](https://github.com/The-Strategy-Unit/azkit/issues) feature on
GitHub to report any bugs, ideas or problems, including with the package
documentation.
